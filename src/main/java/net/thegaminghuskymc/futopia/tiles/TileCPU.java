...
public class TileCPU extends TileBase implements IRedBusWindow {
    ...
    public void powerOnReset() {
        ...
        if (disc == null) {
            Futopia.LOGGER.logInfo("[BluePowerControl] CPU failed to load bootloader " + bootLoader);
            return;
        }

        try {
            Futopia.LOGGER.logInfo("[BluePowerControl] CPU loaded bootloader " + bootLoader);
    ...
    private void dumpMemory() {
        ...
        Futopia.LOGGER.logInfo(message);
    ...
    private void executeInstruction() {
        ...
        Futopia.LOGGER.logInfo("[BluePowerControl] CPU " + Integer.toHexString(this.programCounter) + ":" + Integer.toHexString(opcode));
        this.programCounter++;
    ...
    public void opSTP() {// 65EL02 modified
        availableCycles = -1;
        halt = true;
        Futopia.LOGGER.logInfo("self block=" + this.blockType);
    ...
    private void invalid(int op) {
        this.halt = true;
        Futopia.LOGGER.logInfo("BluePower CPU, Invalid OP code:" + Integer.toHexString(op));
    }
