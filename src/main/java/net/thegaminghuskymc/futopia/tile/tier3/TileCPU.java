...
public class TileCPU extends TileBase implements IRedBusWindow {
    ...
    public void powerOnReset() {
        ...
        if (disc == null) {
            Futopia.log.info("[BluePowerControl] CPU failed to load bootloader " + bootLoader);
            return;
        }

        try {
            Futopia.log.info("[BluePowerControl] CPU loaded bootloader " + bootLoader);
    ...
    private void executeInstruction() {
        ...
        Futopia.log.info("[BluePowerControl] CPU " + Integer.toHexString(this.programCounter) + ":" + Integer.toHexString(opcode));
        this.programCounter++;
    ...
    private void opREP() {
        ...
        setFlags(getFlags() & (~readMemory(this.programCounter)));
    }
    ...
    public void opSTP() {// 65EL02 modified
        availableCycles = -1;
        halt = true;
        Futopia.log.info("self block=" + this.blockType);
        if (this.worldObj.isAirBlock(this.getPos())) {
            //set CPU on fire for the lolz
            this.worldObj.playSound((double) this.getPos().getX() + 0.5D, (double) this.getPos().getY() + 0.5D, (double) this.getPos().getZ() + 0.5D, SoundEvent.REGISTRY.getObject(null), SoundCategory.BLOCKS, 20F, 10F, false);
        }
    }
    ...
    private void invalid(int op) {
        this.halt = true;
        Futopia.log.error("BluePower CPU, Invalid OP code:" + Integer.toHexString(op));
    }
